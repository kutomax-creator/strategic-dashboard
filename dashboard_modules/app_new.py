...
# Components
from dashboard_modules.components.news import fetch_news_for
...
def render():
    ...
    if not reports_ready:
        ...
        if generate_button:
            ...
            kddi_news_raw = fetch_news_for("KDDI", 8)
            ...
            kddi_tuple = tuple(a["title"] for a in kddi_news_raw)
            fujitsu_tuple = tuple(a["title"] for a in fujitsu_news_raw)
            progress_bar.progress(10, text="Analyzing opportunities...")
            opportunities = generate_opportunities(kddi_tuple, fujitsu_tuple)
            ...
            for idx, opp in enumerate(top_opportunities):
                ...
                fname, sec_html, rep_title = generate_detail_report(t, kddi_tuple, fujitsu_tuple)
